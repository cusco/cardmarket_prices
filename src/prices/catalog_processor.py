        ...
        for catalog_file in catalog_files:
            try:
                ...
                if file_date >= filter_date:
                    ...
                    logger.debug(
                        "Skipping file %s (before filter date %s)",
                        catalog_file.name,
                        filter_date,
                    )
            except ValueError:
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if new_prices is not None:
                ...
                if new_prices > 0:
                    logger.info(
                        "Successfully processed %s: %d new prices inserted",
                        catalog_file.name,
                        new_prices,
                    )
                else:
                    logger.info(
                        "Processed %s: no new prices (all already exist)",
                        catalog_file.name,
                    )
                return result
...
def update_cm_prices(local_content=None, force_reprocess=False):
    ...
    if catalog_already_exists and not force_reprocess:
        logger.debug(
            "Catalog with md5 %s already exists, checking for missing individual prices",
            md5sum[:8],
        )
    ...
    if unknown_cards:
        logger.warning(
            "Prices with unknown cards: %d (examples: %s)",
            len(unknown_cards),
            list(unknown_cards)[:5],
        )
